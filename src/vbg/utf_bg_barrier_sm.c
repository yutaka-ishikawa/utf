...
/**
 * SM_REDUCE_OP (Multithreaded version)
...
#define SM_REDUCE_OP(data_1, data_2) {                                              \
    switch (utf_bg_grp->poll_info.sm_utofu_op) {                                    \
        ...
        default:                                                                    \
            utf_printf("%s(SM_REDUCE_OP): Invalid reduction operation:"             \
                       " intra_index=%zu op=%d\n",                                  \
                       __func__, utf_bg_grp->poll_info.sm_intra_index,              \
                       utf_bg_grp->poll_info.sm_utofu_op);                          \
...
/**
 * SM_REDUCE_OP (Non-multithreaded version)
 *   Performs the reduction operation in multiple processes in a node.
 * @param(IN/OUT) data1        A pointer to the operand 1 of operation.
 * @param(IN)     data2        A pointer to the operand 2 of operation.
 */
...
/**
 * utf_poll_barrier_sm (Multithreaded version)
 *   In the soft barrier process, the leader process do the barrier
 *   communication.
 * @param(IN)  utf_bg_grp  A pointer to the structure that stores the VBG information.
 * @return UTF_SUCCESS, UTF_ERR_NOT_COMPLETED, or other return codes of uTofu
 *         functions.
 */
int utf_poll_barrier_sm(utf_coll_group_detail_t *utf_bg_grp)
{
    int      i, rc;
    size_t   num_proc = utf_bg_grp->poll_info.sm_numproc;
    size_t   rank = utf_bg_grp->poll_info.sm_loop_rank;
    uint64_t seq_val = utf_bg_grp->poll_info.sm_seq_val;
    int      loop_counter = utf_bg_grp->poll_info.sm_loop_counter;

    if (!utf_bg_grp->poll_info.sm_flg_comm_start) {
#if defined(DEBUGLOG2)
        fprintf(stderr, "%s: node rank=%zu(%zu) mmap_seq(%zu)=%p(%zu) seq_val=%zu\n",
                __func__, rank, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.sm_intra_index,
                UTF_BG_MMAP_SEQ(utf_bg_grp->poll_info.sm_intra_index),
                *(UTF_BG_MMAP_SEQ(utf_bg_grp->poll_info.sm_intra_index)), seq_val);
#endif
            ...
            if (rank % 2 || (rank + 1) * i >= num_proc) {
                ...
                *(UTF_BG_MMAP_SEQ(rank * i)) = seq_val;
                ...
                fprintf(stderr, "%s: rank=%zu(%zu) set mmap_seq(%zu)=%p(%zu)\n",
                        __func__, rank, utf_bg_grp->poll_info.sm_intra_index, rank*i, UTF_BG_MMAP_SEQ(rank*i),
                ...
                if (*UTF_BG_MMAP_SEQ((rank + 1) * i) != seq_val) {
                    ...
                    utf_bg_grp->poll_info.sm_loop_counter = i;
                    utf_bg_grp->poll_info.sm_loop_rank = rank;
                    return UTF_ERR_NOT_COMPLETED;
                }
#if defined(DEBUGLOG2)
                fprintf(stderr, "%s: rank=%zu(%zu) ref mmap_seq(%zu)=%p(%zu)\n",
                        __func__, rank, utf_bg_grp->poll_info.sm_intra_index, (rank+1)*i,
                        UTF_BG_MMAP_SEQ((rank+1)*i), *UTF_BG_MMAP_SEQ((rank+1)*i));
#endif
                rank >>= 1;
            }
        }

        if (utf_bg_grp->poll_info.utf_bg_poll_ids != -1) {
#if defined(DEBUGLOG2)
            fprintf(stderr, "%s: Call utofu_barrier: node rank=%zu vbg_ids=%016lx\n",
                    __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids);
#endif
            /* The leader process starts the barrier communication */
            if (UTOFU_SUCCESS != (rc = utofu_barrier(utf_bg_grp->poll_info.utf_bg_poll_ids, 0))) {
                return rc;
            }
        }
        utf_bg_grp->poll_info.sm_flg_comm_start = true;
    }
    ...
    if (utf_bg_grp->poll_info.utf_bg_poll_ids != -1) {
        ...
        if (UTOFU_SUCCESS != (rc = utofu_poll_barrier(utf_bg_grp->poll_info.utf_bg_poll_ids, 0))) {
            return rc;
        }
    ...
    fprintf(stderr, "%s: node rank=%zu mmap_seq(0)=%p(%zu)\n",
            __func__, utf_bg_grp->poll_info.sm_intra_index, UTF_BG_MMAP_SEQ(0), *(UTF_BG_MMAP_SEQ(0)));
#endif /* DEBUGLOG2 */
...
/**
 * utf_poll_barrier_sm (Non-multithreaded version)
 *   In the soft barrier process, the leader process do the barrier
 *   communication.
 * @param  none.
 * @return UTF_SUCCESS, UTF_ERR_NOT_COMPLETED, or other return codes of uTofu
...
/**
 * utf_poll_reduce_double_sm (Multithreaded version)
 ...
 *         functions.
 */
int utf_poll_reduce_double_sm(utf_coll_group_detail_t *utf_bg_grp, void **data)
{
    int      rc;
    size_t   i, n;
    size_t   count = utf_bg_grp->poll_info.utf_bg_poll_numcount;
    size_t   num_proc = utf_bg_grp->poll_info.sm_numproc;
    size_t   rank = utf_bg_grp->poll_info.sm_loop_rank;
    size_t   loop_counter = utf_bg_grp->poll_info.sm_loop_counter;
    uint64_t seq_val = utf_bg_grp->poll_info.sm_seq_val;
    ...
    if (!utf_bg_grp->poll_info.sm_flg_comm_start) {
        ...
        fprintf(stderr, "%s: rank=%zu(%zu) op=%d idata=%p count=%zu mmap_seq(%zu)=%p(%zu), seq_val=%zu\n",
                __func__, rank, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_op,
                utf_bg_grp->poll_info.utf_bg_poll_idata, count, utf_bg_grp->poll_info.sm_intra_index,
                UTF_BG_MMAP_SEQ(utf_bg_grp->poll_info.sm_intra_index),
                *(UTF_BG_MMAP_SEQ(utf_bg_grp->poll_info.sm_intra_index)), seq_val);
#endif
        for (i = loop_counter; i < num_proc; i <<= 1) {
            if (rank % 2 || (rank + 1) * i >= num_proc) {
                /* Odd rank, or even rank without pair stores the value. */
                for (n = 0; n < count; n++) {
                   *(UTF_BG_MMAP_BUF(rank * i) + n)  = *((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_idata + n);
                }
                utf_bg_wmb();
                *(UTF_BG_MMAP_SEQ(rank * i)) = seq_val;
#if defined(DEBUGLOG2)
                ...
                for (n = 0; n < count; n++) {
                    fprintf(stderr, "%s: rank=%zu(%zu) pair=%zu op=%d type=0x%lx count=%zu/%zu before idata=%lf MMAP_BUF=%lf\n",
                            __func__, rank, utf_bg_grp->poll_info.sm_intra_index, (rank+1)*i,
                            utf_bg_grp->poll_info.utf_bg_poll_op, utf_bg_grp->poll_info.utf_bg_poll_datatype, n+1, count,
                            *((double *)utf_bg_grp->poll_info.utf_bg_poll_idata + n),
                ...
                for (n = 0; n < count; n++) {
                    fprintf(stderr, "%s: rank=%zu(%zu) op=%d count=%zu/%zu after idata=%lf\n",
                            __func__, rank, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_op,
                            n+1, count, *((double *)utf_bg_grp->poll_info.utf_bg_poll_idata + n));
                }
#endif /* DEBUGLOG2 */
                rank >>= 1;
            }
        ...
        if (utf_bg_grp->poll_info.utf_bg_poll_ids != -1) {
            ...
            fprintf(stderr, "%s: Call utofu_reduce_double: node rank=%zu vbg_ids=%016lx op=%d, count=%zu\n",
                    __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids,
                    utf_bg_grp->poll_info.sm_utofu_op, count);
            for (n = 0; n < count; n++) {
                fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx count=%zu/%zu arg data[%zu]=%lf\n",
                        __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids, n+1,
                        count, n, *((double *)utf_bg_grp->poll_info.utf_bg_poll_idata+n));
            }
#endif /* DEBUGLOG2 */
            /* The leader process starts the barrier communication */
            if (UTOFU_SUCCESS !=
                (rc = utofu_reduce_double(utf_bg_grp->poll_info.utf_bg_poll_ids,
                                          utf_bg_grp->poll_info.sm_utofu_op,
                                          utf_bg_grp->poll_info.utf_bg_poll_idata, count, 0))) {
                return rc;
            }
        }
        utf_bg_grp->poll_info.sm_flg_comm_start = true;
    ...
    if (utf_bg_grp->poll_info.utf_bg_poll_ids != -1) {
        ...
        fprintf(stderr, "%s: Call utofu_poll_reduce_double: node rank=%zu vbg_ids=%016lx odata=%p\n",
                __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids,
                utf_bg_grp->poll_info.utf_bg_poll_odata);
#endif
        /* The leader process waits for the communication to complete. */
        if (UTOFU_SUCCESS !=
            (rc = utofu_poll_reduce_double(utf_bg_grp->poll_info.utf_bg_poll_ids, 0,
                                           (double *)utf_bg_grp->poll_info.utf_bg_poll_odata))) {
            return rc;
        }
        /* Store the calculation results in shared memory. */
        for (n = 0; n < count; n++) {
            *((double *)UTF_BG_MMAP_BUF(0) + n) = *((double *)utf_bg_grp->poll_info.utf_bg_poll_odata + n);
        }
#if defined(DEBUGLOG2)
        ...
        for (n = 0; n < count; n++) {
            fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx count=%zu/%zu odata[%zu]=%lf\n",
                    __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids, n+1,
                    count, n, *((double *)utf_bg_grp->poll_info.utf_bg_poll_odata + n));
        }
#endif /* DEBUGLOG2 */
        utf_bg_wmb();
        ...
        *(UTF_BG_MMAP_SEQ(0)) = seq_val;
        ...
        for (n = 0; n < count; n++) {
            *((double *)utf_bg_grp->poll_info.utf_bg_poll_odata + n) = *((double *)UTF_BG_MMAP_BUF(0) + n);
#if defined(DEBUGLOG2)
            fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx count=%zu/%zu odata[%zu]=%lf\n",
                    __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids, n+1,
                    count, n, *((double *)utf_bg_grp->poll_info.utf_bg_poll_odata + n));
    ...
    fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx data=%p poll_result=%p root=%d count=%zu size=%zu\n",
            __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids, data, 
            utf_bg_grp->poll_info.utf_bg_poll_result, utf_bg_grp->poll_info.utf_bg_poll_root,
    ...
    switch (utf_bg_grp->poll_info.utf_bg_poll_size) {
        ...
        default:
            utf_printf("%s: Invalid data size: intra_index=%zu size=%zu\n",
                       __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_size);
            return UTF_ERR_INTERNAL;
    }
...
/**
 * utf_poll_reduce_double_sm (Non-multithreaded version)
 *   In the soft barrier process, values for all proccesses in a node are
 *   collected to the leader process, and the leader process do the barrier
 *   communication.
 *   The worker processes confirm that the barrier communication of the 
...
/**
 * utf_poll_reduce_uint64_sm (Multithreaded version)
 *   In the soft barrier process, values for all proccesses in a node are
 *   collected to the leader process, and the leader process do the barrier
 *   communication.
 *   The worker processes confirm that the barrier communication of the 
 ...
 *         functions.
 */
int utf_poll_reduce_uint64_sm(utf_coll_group_detail_t *utf_bg_grp, void **data)
{
    int      rc;
    size_t   i, j, n;
    size_t   count = utf_bg_grp->poll_info.utf_bg_poll_numcount;
    ...
    if (!utf_bg_grp->poll_info.sm_flg_comm_start) {
        ...
        fprintf(stderr, "%s: rank=%zu(%zu) op=%d utofu_op=%d idata=%p count=%zu mmap_buf=%p mmap_seq(%zu)=%p(%zu) seq_val=%zu\n",
                __func__, rank, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_op,
                utf_bg_grp->poll_info.sm_utofu_op,
                utf_bg_grp->poll_info.utf_bg_poll_idata,  count, utf_bg_grp->poll_info.sm_mmap_buf,
                utf_bg_grp->poll_info.sm_intra_index, UTF_BG_MMAP_SEQ(utf_bg_grp->poll_info.sm_intra_index),
                *(UTF_BG_MMAP_SEQ(utf_bg_grp->poll_info.sm_intra_index)), seq_val);
#endif
        for (i = loop_counter; i < num_proc; i <<= 1) {
            if (rank % 2 || (rank + 1) * i >= num_proc) {
                ...
                *(UTF_BG_MMAP_SEQ(rank * i)) = seq_val;
                ...
                for (n = 0; n < count; n++) {
                    fprintf(stderr, "%s: rank=%zu(%zu) pair=%zu utofu_op=%d type=0x%lx count=%zu/%zu"
                            ...
                            __func__, rank, utf_bg_grp->poll_info.sm_intra_index, (rank+1)*i,
                            utf_bg_grp->poll_info.sm_utofu_op, utf_bg_grp->poll_info.utf_bg_poll_datatype,
                            n+1, count, *((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_idata + n),
                            *(UTF_BG_MMAP_BUF((rank+1)*i) + n));
                }
#endif /* DEBUGLOG2 */
                /* Operate the own value and the value of next rank. */
                SM_REDUCE_OP((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_idata, UTF_BG_MMAP_BUF((rank + 1)*i));
#if defined(DEBUGLOG2)
                for (n = 0; n < count; n++) {
                    fprintf(stderr, "%s: rank=%zu(%zu) utofu_op=%d count=%zu/%zu after idata=0x%lx\n",
                            __func__, rank, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.sm_utofu_op,
                            n+1, count, *((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_idata + n));
                }
#endif /* DEBUGLOG2 */
                rank >>= 1;
        ...
        if (utf_bg_grp->poll_info.utf_bg_poll_ids != -1) {
            ...
            for (n = 0; n < count; n++) {
                fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx count=%zu/%zu arg data[%zu]=0x%lx\n",
                        __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids,
                        n+1, count, n, *((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_idata+n));
            }
#endif /* DEBUGLOG2 */
            /* The leader process starts the barrier communication */
            if (UTOFU_SUCCESS !=
                (rc = utofu_reduce_uint64(utf_bg_grp->poll_info.utf_bg_poll_ids,
                                          utf_bg_grp->poll_info.sm_utofu_op,
                                          utf_bg_grp->poll_info.utf_bg_poll_idata, count, 0))) {
                return rc;
            }
    ...
    if (utf_bg_grp->poll_info.utf_bg_poll_ids != -1) {
        ...
        /* The leader process waits for the communication to complete. */
        if (UTOFU_SUCCESS !=
            (rc = utofu_poll_reduce_uint64(utf_bg_grp->poll_info.utf_bg_poll_ids, 0,
                                           (uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_odata))) {
            return rc;
        }
        /* Store the calculation results in shared memory. */
        for (n = 0; n < count; n++) {
            *(UTF_BG_MMAP_BUF(0) + n) = *((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_odata + n);
        }
#if defined(DEBUGLOG2)
        fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx now mmap_seq(0)=%p(%zu)\n",
                __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids,
        ...
        for (n = 0; n < count; n++) {
            fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx count=%zu/%zu odata[%zu]=0x%lx\n",
                    __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids,
                    n+1, count, n, *((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_odata + n));
        }
#endif /* DEBUGLOG2 */
        utf_bg_wmb();
        ...
        *(UTF_BG_MMAP_SEQ(0)) = seq_val;
        ...
        /* Gets the leader's result from shared memory. */
        for (n = 0; n < count; n++) {
            *((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_odata + n) = *(UTF_BG_MMAP_BUF(0) + n);
#if defined(DEBUGLOG2)
            fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx count=%zu/%zu odata[%zu]=0x%lx\n",
                    __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids,
                    n+1, count, n, *((uint64_t *)utf_bg_grp->poll_info.utf_bg_poll_odata + n));
    ...
    fprintf(stderr, "%s: node rank=%zu vbg_ids=%016lx data=%p poll_result=%p root=%d count=%zu op=%d datatype=0x%lx\n",
            __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_ids, data,
            utf_bg_grp->poll_info.utf_bg_poll_result, utf_bg_grp->poll_info.utf_bg_poll_root,
            utf_bg_grp->poll_info.utf_bg_poll_count,
            utf_bg_grp->poll_info.utf_bg_poll_op, utf_bg_grp->poll_info.utf_bg_poll_datatype);
#endif
    /* Set the result in the user buffer. */
    switch (utf_bg_grp->poll_info.utf_bg_poll_op) {
        case UTF_REDUCE_OP_SUM:
        ...
        case UTF_REDUCE_OP_MIN:
            if(UTF_DATATYPE_DIV_REAL == utf_bg_grp->poll_info.utf_bg_poll_datatype >> 24){
        ...
        default:
            utf_printf("%s: Invalid reduction operation: intra_index=%zu op=%d\n",
                       __func__, utf_bg_grp->poll_info.sm_intra_index, utf_bg_grp->poll_info.utf_bg_poll_op);
            return UTF_ERR_INTERNAL;
    }
...
/**
 * utf_poll_reduce_uint64_sm (Non-multithreaded version)
 *   In the soft barrier process, values for all proccesses in a node are
 *   collected to the leader process, and the leader process do the barrier
 *   communication.
 *   The worker processes confirm that the barrier communication of the 
